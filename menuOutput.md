...
<p>Установленный флажок Separate Pages? позволяет распечатывать каждый слой, выбранный в разделе Layers/Artworks на отдельном листе. Если флажок не установлен все выбранные слои накладываются друг на друга и печатаются на одном листе.</p>
<p>Параметр Copies определяет количество копий при печати.</p>
<p>Раздел Rotation определяет ориентацию изображения относительно страницы:</p>
<p>X Horizontal – ось X параллельна краю листа меньшему по размеру и направлена вправо.</p>
<p>X Vertical – ось X параллельна краю листа большему по размеру и направлена вверх.</p>
...
<p>Корректирующие коэффициенты – Compensation Factors – масштабируют абсолютные координаты графики, не изменяя относительных размеров. Например, если существует линия длиной 10 условных единиц с началом в координатах (5, 5) и установлены корректирующие коэффициенты X = 0,6 и Y = 0,5, то линия будет выводиться в точке с координатами (3, 2), но её длина (10 условных единиц) не изменится. Эти коэффициенты можно использовать при печати изображений, для которых критически важным является сохранение относительного размера графики. Коэффициенты могут изменяться в пределах от 0,01 до 100.</p>
<p>Установленный флажок напротив пути в разделе Print to File? позволяет распечатать изображение в файл, а также выбрать путь и название файла (с помощью кнопки Filename).</p>
<p>Меню Advanced Option позволяет всегда использовать цветные настройки для принтера (Always Enable Color Options) и всегда использовать драйвер для графопостроителя Labcenter (Always Enable Labcenter Plotter Support).</p>
...
<ol>
<li>Position Output at Top – выровнять печатаемое изображение по верхнему краю;</li>
<li>Position Output at Bottom – выровнять печатаемое изображение по нижнему краю;</li>
<li>Position Output at Left – выровнять печатаемое изображение по левому краю;</li>
<li>Position Output at Right – выровнять печатаемое изображение по правому краю;</li>
<li>Position Output at Center – выровнять печатаемое изображение по центру;</li>
<li>Position Output Numerically… – позволяет задать отступы сверху, снизу, слева и справа от печатаемого изображения;</li>
<li>Use Printer Margins? – использовать поля принтера или нет.</li>
</ol>
<p>Пользователь может вручную настроить положение печатной платы в пределах области печати, нажав правую кнопку мыши в окне предварительного просмотра и выбрав Position Output Numerically… и введя отступ между границей области печати и границей печатной платы. Это абсолютный метод позиционирования печатной платы в пределах области печати. В общем случае большинству пользователей будет проще задавать положение печатной платы, перетаскивая плату с помощью мыши.</p>
<p>Если снять флажок Use Printer Margins? в контекстном меню, то отступы, которые определил пользователь, будут отсчитываться от границы самой бумаги, а не области печати (Рис. 9). В большинстве случаев это делать не рекомендуется поскольку, в зависимости от модели принтера, можно потерять часть печатаемого изображения, если расположить его у края бумаги (вне области печати).</p>
